...
const { generateUniqueFilename } = require("../config/multer.cjs");
...
class FileService {
  ...
  static async getFilesByCategory(category) {
    ...
    const dbCategory = categoryMapping[category] || "gallery";
    
    const result = await query(
      "SELECT id, original_name, filename, file_path, file_size, mime_type, category, uploaded_by, created_at FROM uploaded_files WHERE category = $1 ORDER BY created_at DESC",
      [dbCategory]
    );
    return result.rows;
  }
